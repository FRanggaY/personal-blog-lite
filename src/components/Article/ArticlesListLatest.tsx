...
const ArticleListLatest: React.FC<ArticleListLatestProps> = ({ articles, title, language }) => {
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row md:space-x-4">
        ...
        <Link href={`/${language}/articles/${latestArticles[0].slug}`}>
        ...
        <div className="flex flex-col md:flex-row md:flex-wrap">
          {latestArticles.slice(1).map(article => (
            <Link href={`/${language}/articles/${article.slug}`} key={article.slug}>
